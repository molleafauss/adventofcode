...
public class Day16opt implements Solver {
...
class OnePathSolver {
    ...
    public OnePath findPath(OnePath path) {
        ...
        for (var valve : valvesWithFlow) {
            ...
            if (next.elapsed >= Day16opt.PART1_MINUTES) {
                continue;
            }
...
class OnePath {
    ...
    public OnePath next(Valve valve, byte distance) {
        ...
        var flow = (Day16opt.PART1_MINUTES - elapsed) * valve.flow;
        return new OnePath(
                visited,
...
class TwoPathSolver {
    ...
    public TwoPath findPath(TwoPath path) {
        ...
        for (var valve : valvesWithFlow) {
            ...
            if (next.elapsed < Day16opt.PART2_MINUTES) {
                var subBest = findPath(next);
                if (subBest.totalFlow > bestPath.totalFlow) {
                    bestPath = subBest;
                }
...
class TwoPath {
    ...
    public TwoPath nextHuman(Valve valve, int distance) {
        ...
        var flow = (Day16opt.PART2_MINUTES - elapsed) * valve.flow;
        return new TwoPath(
                humPath,
                (byte) (humanPathPos + 1),
                (byte) elapsed,
