...
Some tweaks were made which resulted in some surprising results:
- using a record or hand-crafting a long (that has to be boxed into a `Long`) for the cache key 
  gives an unexpected few seconds advantage to the record. The native implementation of a record 
  hashCode provided by the JVM may be optimised enough that the bit manipulation plus the boxing 
  doesn't produce any advantage.
...
The above experiments were interesting to validate if readable code using standard JDK data 
structures results in significant lesser performance than using "tweaks" with primitives - 
doesn't seem so, reinforcing that premature optimisations because the programmer feels "better in 
control" aren't really giving advantage (but they are making the code harder).
