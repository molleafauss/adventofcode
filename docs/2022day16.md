...
Moving out the strings from the Java program improved execution time to around 140sec, about 
30% less time.
...
profiling; it's no surprising that the cache operations are those consuming the most 
...
We probably want a specialised structure for caching.
