...
(*) Rust takes ~30s to output the final result, due to running Drop on all allocated entities in
    the cache. Funny enough, this delay happens doesn't show up in Mac.
...
## Any optimization we should try?
...
A first round of optimisations was applied to the Java version:
...
The results of those optimizations are paying off, the algorithm runs in ~75-80 sec, about 40% 
or less than the original time. 
...
- The default G1 garbage collector is faster than the newer ZGC and the old Parallel GC (~15sec 
  difference).
- using Arrays.copyOf instead of System.arrayCopy in the TwoPath next/merge/diff result in ~10sec 
  longer run time; more data gets copied around, apparently.


## Applying first optimisation round to all languages
Re-using the Java algorithm, now, we can try seeing how the other languages fare.

| LANGUAGE | SOLUTION TIME | CPU USAGE           | MEMORY PEAK         |
|----------|---------------|---------------------|---------------------|
| Python   | 410.7sec      | avg & peak 100%     | 19GB                | 
| Rust     | 80.0sec (*)   | avg & peak 100%     | 4.5GB (peaks at 6)  | 
| Java     | 85.5sec       | avg 120%, peak 250% | 8.5GB               | 
| Go       | 77.6sec       | avg 100%, peak 500% | 10.2GB (peak at 19) |
(*) Rust deallocation phase takes now ~12sec down from 30.

Notes:
- in rust like in java, using an unnamed struct or a long for the cache key doesn't give any 
  noticeable performance improvement.
- Go still has the lead, however, the gap is vastly reduced now, and Rust is faster than Java.
- Python doesn't see any particular improvement, memory is actually higher.

It seems we're heading in the right direction. Let's stop optimising python, the language is anyway 
interpreted, so any effort with the sheer number of operations the algorithm requires can't 
really be optimised more than needed.
